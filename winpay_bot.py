...
broadcast_config = {}  # {chat_id: {"team": str, "file_id": str, "ad_text": str, "time": str}}，存储群发配置

# 设置日志任务
def setup_schedule():
    schedule.every().day.at("00:00").do(lambda: asyncio.run(job()))
    for config in broadcast_config.values():
        if "time" in config and config["time"] != "-1":
            schedule.every().day.at(config["time"]).do(send_broadcast, config["team"])
...
查看操作员：操作员列表
        ...
        elif message_text.startswith("编队") or message_text.startswith(tuple(f"{team} " for team in teams.keys())) or message_text.startswith(("群发", "查询群发", "预览", "ID", "群发说明")):
            if username and username in operators.get(chat_id, {}):
                if message_text.startswith("编队"):
                    ...
                    if update.message.reply_to_message and username in operators.get(chat_id, {}):
                        ...
                        if reply_msg.animation:
                            ...
                            broadcast_config[chat_id] = {"team": None, "file_id": file_id, "ad_text": ad_text, "time": None}
                            ...
                            if team_name in teams:
                                if time_str == "-1":
                                    ...
                                    try:
                                        ...
                                        schedule.clear()
                                        setup_schedule()  # 重新加载调度
                                        await update.message.reply_text(f"已为 {team_name} 队设置群发: {time_str}")
                                    except ValueError:
                                        await update.message.reply_text("时间格式错误，请使用 HH:MM")
                            else:
                                await update.message.reply_text(f"队 {team_name} 不存在")
                        else:
                            await update.message.reply_text("格式错误，请使用：群发 队名 时间 或 群发 队名 -1")
                    else:
                        await update.message.reply_text("请回复包含动图或视频的消息")
                    ...
                    if broadcast_config:
                        ...
                        for team, config in broadcast_config.items():
                            if config["time"] != "-1":
                                response += f"{team} - {config['time']} - [{'视频' if config['file_id'].startswith('video') else '动图'}预览] {config['file_id']}\n{config['ad_text']}\n"
                        await update.message.reply_text(response.strip())
                    else:
                        await update.message.reply_text("暂无群发设置")
                elif message_text == "预览":
                    print("匹配到 '预览' 指令")
                    if chat_id in broadcast_config and broadcast_config[chat_id]["time"] != "-1":
        ...
        # 记账相关指令（私聊或群组均有效）
            ...
            if username and username in operators.get(chat_id, {}):
                ...
                try:
                    ...
                    if amount_str.lower().endswith('u'):
                        ...
                        transaction = f"入款 {format_amount(amount)}u {timestamp} -> {format_amount(amount)}u (由 {operator_name})"
                        ...
                        transaction = f"入款 {format_amount(amount)} {timestamp} -> {format_amount(adjusted_amount)}u (由 {operator_name})"
                    else:
            ...
            if username and username in operators.get(chat_id, {}):
                ...
                try:
                    ...
                    if amount_str.lower().endswith('u'):
                        amount = float(amount_str.rstrip('uU'))
                        transaction = f"下发 {format_amount(amount)}u {timestamp} -> {format_amount(amount)}u (由 {operator_name})"
                    elif amount_str.endswith(".0") or amount_str.endswith(".00"):
                        amount = float(amount_str.rstrip('.0').rstrip('.00'))
                        adjusted_amount = amount * (1 + withdraw_fee_rate) / exchange_rate_withdraw
                        transaction = f"下发 {format_amount(amount)} {timestamp} -> {format_amount(adjusted_amount)}u (由 {operator_name})"
                    else:
                        amount = float(amount_str)
                        adjusted_amount = amount * (1 + withdraw_fee_rate) / exchange_rate_withdraw
                        transaction = f"下发 {format_amount(amount)} {timestamp} -> {format_amount(adjusted_amount)}u (由 {operator_name})"
